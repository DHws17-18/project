...
aiFile = open('dblp.ai.csv','r')
allFile = open('dblp.all.csv','r')
...
if __name__ == '__main__':
  reader = csv.reader(aiFile, delimiter=';')
  ...
  try:
    x, y = zip(*l)
    line, = plt.plot(x, y, label=" all keywords")
    handles.append(line)
    aiFile.seek(0)
  except:
    print()
  ...
  years = blankyears
  reader = csv.reader(allFile, delimiter=';')
  for row in reader:
    if not row[0] in years:
      years[row[0]]+=1
  l = sorted(years.items()) 
  print(l)
  x, y = zip(*l)
  line, = plt.plot(x, y, label=" all Publications")
  handles.append(line)



  ticks = list(range(1930, 2015, 5))
  plt.xticks(ticks)
